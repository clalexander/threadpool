...
data "template_file" "execution_policy" {
  ...
  vars = {
    ...
    INKSOFT_ORDERS_TABLE_NAME = var.inksoft_orders_table_name
...
module "event-processor" {
  ...
  service_name = "Threadpool_InkSoft_OrderShipmentsWriteback"

  lambda_filename =  "${var.packages_dir}/inksoft-order-shipments-writeback.zip"
